...
let flows = model.findFlows();

// Create a header row containing the attributes of a Stock
let header = ['id', 'name', 'flowRate', 'unit', 'onlyPositive', 'startStock', 'endStock', 'note'];

let tsvData = header.join('\t') + '\n';

flows.forEach((flow, index) => {
  const startStock = flow.start;
  const endStock = flow.end;

  const startStockID = startStock ? startStock.id : null;
  const endStockID = endStock ? endStock.id : null;
  let flowData = [flow.id, flow.name, flow.rate.replace(/\n/g, ''), flow.units, flow.nonNegative, startStockID, endStockID, flow.note];
  tsvData += flowData.join('\t') + '\n';

  console.log(`Flow ${index + 1}: ${flow.id}, ${flow.name}, ${flow.rate}, ${flow.units}, ${flow.nonNegative}, ${startStockID}, ${endStockID}, ${flow.note}`);
});

// Create TSV file containing Flow data
fs.appendFile('flowData.tsv', tsvData, function (err) {
  if (err) throw err;
