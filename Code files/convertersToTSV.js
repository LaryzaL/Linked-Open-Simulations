...
let converters = model.findConverters();

let header = ['id', 'name', 'note', 'inputSource', 'data', 'unit', 'interpolation'];

let tsvData = header.join('\t') + '\n';

//Source van data --> Uitbreiding project met csv on the web
converters.forEach((converter, index) => {
  
  let converterData = [converter.id, converter.name, converter.note, converter._node.getAttribute("Source"), JSON.stringify(converter.values), converter.units, converter.interpolation];
  tsvData += converterData.join('\t') + '\n';

  console.log(`Converter ${index + 1}:${converter._node.getAttribute("Source")}`);
});

// Create TSV file containing Converter data
fs.appendFile('converterData.tsv', tsvData, function (err) {
  if (err) throw err;
