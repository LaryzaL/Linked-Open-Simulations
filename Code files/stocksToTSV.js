...
let stocks = model.findStocks();

// Create a header row containing the attributes of a Stock
let header = ['id', 'name', 'initialValue', 'unit', 'delay', 'nonNegative', 'note'];

let tsvData = header.join('\t') + '\n';

// For each found Stock, display its selected attributes
stocks.forEach((stock, index) => {

  let stockData = [stock.id, stock.name, stock.initial.replace(/\n/g, ''), stock.units, stock.delay, stock.nonNegative, stock.note];
  tsvData += stockData.join('\t') + '\n';

  console.log(`Stock ${index + 1}: ${stock.id}, ${stock.name}, ${stock.initial.replace(/\n/g, '')}, ${stock.units}, ${stock.delay}, ${model.globals}`);
});

// Create TSV file containing Stock data
fs.appendFile('stockData.tsv', tsvData, function (err) {
  if (err) throw err;
