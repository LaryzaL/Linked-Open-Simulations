...
let variables = model.findVariables();

let header = ['id', 'name', 'equation', 'unit', 'note'];

let tsvData = header.join('\t') + '\n';

variables.forEach((variable, index) => {

  let variableData = [variable.id, variable.name, variable.value.replace(/\n/g, ''), variable.units, variable.note];
  tsvData += variableData.join('\t') + '\n';

  console.log(`Variable ${index + 1}: ${variable.name}, ${variable.value}`);
  });

  // Create TSV file containing Variable data
fs.appendFile('variableData.tsv', tsvData, function (err) {
  if (err) throw err;
